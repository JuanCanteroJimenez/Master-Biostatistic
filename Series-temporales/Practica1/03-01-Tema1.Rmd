...
Libros es una _serie anual_ de 1993 a 2018 (fuente [Instituto Nacional de Estadística](https://www.ine.es)). Los datos están disponibles en el fichero `libros.csv`. La primera columna tiene el año de la serie y la segunda contiene el número de títulos publicados. En la primera fila aparece el nombre de cada columna.
...
Electricidad es una _serie diaria_ desde el 1 enero de 2019 hasta el 31 diciembre de 2019 (fuente: [Red Electrica de España](http://www.ree.es)). Los datos están disponibles en el fichero `Consumo electrico.csv`. La primera columna tiene la fecha y la segunda, la seleccionada para el fechado, la serie propiamente. En la primera fila aparece el nombre de cada columna.
...
* Con `start` indicamos que el primer dato es un martes (segundo día de la semana) de la primera semana. Como ves no hay referencia al año. También sería correcto `start = 1 + 1/7`.
...
grid.arrange(
  ...
  layout_matrix = rbind(c(1,1,1),
                        c(2,3,4),
                        c(5,6,7)),
  top = textGrob("Figura 5. Ejemplos de tendencia", hjust = 1.5)
)
```
...
autoplot(nacimientos,
         xlab = "",
         ylab = "Nacimientos",
         main = "Figura 7. Ciclos en la serie Nacimientos") +
...
autoplot(electricidadb,
         xlab = "",
         ylab = "MWh",
         main = "Figura 9. Consumo diario de electricidad") + 
...
electricidadh <- ts(electricidadh[, 2],
                  start = 1,
                  frequency = 1)
electricidadhb <- window(electricidadh, start = 745, end = 1416)

autoplot(electricidadhb,
         xlab = "",
         ylab = "MW",
         main = "Figura 10. Consumo por hora de electricidad")  + 
  scale_x_continuous(breaks = seq(745,1416,24), labels = 1:28) 
```
...
* _Efectos calendario_: festivos en series diarias; Semana Santa, días laborales y febrero bisiesto en series mensuales.
...
```{r, echo = FALSE}
electricidadb <- window(electricidad, start = c(43, 1))
autoplot(electricidadb,
         xlab = "",
         ylab = "MWh",
         main = "Figura 12. Consumo diario de electricidad") + 
  scale_x_continuous(breaks= 43:53, labels = c("21/10", "28/10", "4/11", "11/11", "18/11", "25/11", 
                                               "2/12", "9/12", "16/12", "23/12", "30/12")) + 
  geom_point(aes(x = 44+4/7, y = electricidadb[12], colour = "red"))  +
  geom_point(aes(x = 43+4/7, y = electricidadb[5], colour = "green")) +
  geom_point(aes(x = 45+4/7, y = electricidadb[19], colour = "green")) +
  geom_point(aes(x = 52+2/7, y = electricidadb[66], colour = "blue")) +
...
* _Pulso (Additive Outlier)_: En un periodo aislado la serie toma un valor anómalo. Por ejemplo, un día el cajero automático no funciona y la extracción de dinero es nula. Véase figura 13, panel derecho.
...
* _Cambio permanente (Level Shift)_: En un periodo la serie cambia de nivel y permanece de forma permanente en este nuevo nivel. Por ejemplo, enfrente de una tienda abre la competencia, de forma que sus ventas descienden bruscamente de forma permanente. Véase figura 13, panel izquierdo.
...
ggplot(datos, aes(x=x, y=y)) + 
  ...
  scale_x_continuous(breaks=NULL) + 
  scale_y_continuous(breaks=NULL) +
  ggtitle("Figura 13. Ejemplos de intervención")
```
...
grid.arrange(
  grobs = list(p1, p2),
  nrow = 2,
  top = textGrob("Figura 14. Ejemplo de esquema aditivo"))
```
...
grid.arrange(
  grobs = list(p1, p2),
  nrow = 2,
  top = textGrob("Figura 15. Ejemplo de esquema multiplicativo"))
```
...
autoplot(nacimientosAnual/1000,
         ...
         main = "Figura 16. Nacimientos por año")
```
...
autoplot(electricidadSemanal,
         xlab = "",
         ylab = "MWh",
         main = "Figura 17. Consumo de electricidad por semana")
```
...
Podemos hacer una gráfico de la serie _contra_ cada periodo estacional. Este gráfico permite identificar el efecto de la estacionalidad en la serie y su evolución en el tiempo. Existen varias opciones para este tipo de gráficos, veremos dos de ellas: gráfico de subseries y gráfico de líneas. Para facilitar la interpretación vamos a trabajar con la serie desde el año 2000 que denominaremos _nacimientosb_.

Veamos primero un ejemplo de gráfico de subseries (figura 18) que muestra para cada periodo estacional la subserie de valores de ese periodo y el valor medio en la subserie. Para las series con tendencia y esquema multiplicativo, el valor medio de las subseries (líneas horizontales) puede llevarnos a una interpretación incorrecta de la estacionalidad. (La función `monthplot` de `stats` realiza un gráfico similar.)
...
ggseasonplot(nacimientosb, 
             ...
             xlab = "",
             ylab = "Nacimientos",
             main = "Figura 19. Gráfico estacional de lineas")
```
...
La siguiente sintaxis usa la función `tapply` para estimar numéricamente la componente estacional. Básicamente, calcula para cada mes (argumento `cycle(nacimientosb)`) la media (`FUN = mean`) del cociente `nacimientosb / mean(nacimientosb)`.
...
Los cambios necesarios para estimar la componente estacional bajo un esquema aditivo son mínimos. Veámoslo para la serie Electricidad
...
En domingo la demanda de electricidad cae en 89 MWh respecto de la media mensual. De lunes a viernes la demanda eléctrica es mayor que la media semanal.
...
autoplot(xx, series="s.window = 'periodic'",
         ylab = "Componente estacional",
         main = "Figura 25. Componente estacional para Electricidad") +
